...
p0 = exp (B);
Y_AL_b =  p0 * exp(A * X); % Recta de Ajuste evaluada en los puntos xk
F2=figure(2);
set(F2,'position',[140 20 1100 650],'Menubar','none',...
        'NumberTitle','off','name', 'Ejercicio 3b TP#FINAL. Métodos Numéricos');
plot(X,Y,'*r',X,Y_AL_b, 'b') % Nube de puntos originales
xlabel('D [mm]')
ylabel('P(D) [mmHg]')
...
[D,err,relerr,n] = Extrapolacion_Richardson_O2n(@(X)  p0 * exp(A * X),D_m,Delta,Tol);% referencia a esa función
F7=figure(7);
set(F7,'position',[140 20 1100 650],'Menubar','none',...
        'NumberTitle','off','name', 'Ejercicio 3f TP#FINAL. Métodos Numéricos');
Elasticidad_2 = D(n+1,n+1);
tangente = Elasticidad_2*(X - D_m) +  p0 * exp(A * D_m);
plot(X,Y,'*r',X,Y_AL_b, 'k',X, tangente, 'm--') % Nube de puntos originales
xlabel('D [mm]')
ylabel('P(D) [mmHg]');
